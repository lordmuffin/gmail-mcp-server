...
# ---------- 4. Auth probe (the big v2 change) ----------
detect_op_mode() {
  # Returns: "service-account", "interactive", or "none" via stdout
...
probe_op_auth() {
  ...
  case "$mode" in
    ...
    service-account-file)
      ...
      # Source the file for the test only
      local out err_out
      err_out="$(mktemp)"
  ...
  case "$mode" in
    service-account-file)
      resolved="$( set -a; source "$SA_FILE"; set +a;
                   cd "$REPO_DIR" && "$OP_BIN" run --no-masking -- printenv GOOGLE_CLIENT_ID 2>"$err_out" )"
      rc=$?
      ;;
    *)
      resolved="$(cd "$REPO_DIR" && "$OP_BIN" run --no-masking -- printenv GOOGLE_CLIENT_ID 2>"$err_out")"
      rc=$?
      ;;
  esac
  if (( rc != 0 )); then
...
${ENV_FILE_DIRECTIVE}
ExecStart=${OP_BIN} run --no-masking -- ${NODE_BIN} --no-warnings dist/index.js
Restart=on-failure
RestartSec=5
